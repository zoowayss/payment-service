package com.coldlake.app.payment.domain.payment.paypal.orders.enums;
...
public enum ThirdPartOrderStatusAdapterEnum {
    ...
    private static final Map<String, ThirdPartOrderStatusAdapterEnum> cache = Arrays
            .stream(com.coldlake.app.payment.domain.payment.paypal.orders.enums.ThirdPartOrderStatusAdapterEnum.values())
    ...
    public static ThirdPartOrderStatusAdapterEnum of(String name) {
        return Optional.ofNullable(ofNullable(name))
                .orElseThrow(() -> new RuntimeException(String.format("%s not found ThirdPartOrderStatusAdapterEnum via string '%s' ", cm(), name)));
    }
