...
import static com.coldlake.app.payment.domain.payment.constants.Constants.ONE_SECOND_MILL;
import static com.coldlake.app.payment.service.payment.LogService.cm;
import static com.coldlake.app.payment.service.payment.PaymentOrderHandler.METADATA_FIELD_ORDER_ID;
...
public class IStripeClient {
    ...
    public Session createSession(PaymentOrderCreation p, SessionCreateParams.Mode mode) throws StripeException {
        ...
        SessionCreateParams params = SessionCreateParams.builder().setMode(mode).putMetadata(METADATA_FIELD_ORDER_ID, p.getOrderId())
                .addLineItem(SessionCreateParams.LineItem.builder().setPrice(p.getSkuId()).setQuantity(p.getQuantity()).build())
                ...
                .setExpiresAt(p.getExpiresAt() / ONE_SECOND_MILL).build();
