package com.coldlake.app.payment.service.paypal.impl;
...
import com.coldlake.app.payment.service.paypal.PaypalClient;
...
public class PaypalServiceImpl implements PaypalService {
    ...
    @Override
    public boolean webhookValidate(Map<String, Object> webhookEvent, String authLog, String certUrl, String transmissionId, String transmissionSig, String transmissionTime) {
    ...
    public void dealOrderEvent(PayPalWebhookEvent body) {
        ...
        PayPalWebhookEvent.Resource resource = Optional.ofNullable(body.getResource()).orElseThrow(() -> new IllegalArgumentException(cm + " can not found field: resource "));
        switch (eventType) {
            case BILLING_SUBSCRIPTION_CANCELLED:
                // BILLING_SUBSCRIPTION_CANCELLED 事件
                String subscriptionId = resource.getId();
                if (!StringUtils.hasText(subscriptionId)) throw new IllegalArgumentException(cm + " can not found subscriptionId");
//                paymentCallbackHandlerWrapper.handleBillingSubscriptionCancelled(subscriptionId);
            ...
            case CHECKOUT_ORDER_APPROVED:
                ...
                if (!StringUtils.hasText(tradeNo)) throw new IllegalArgumentException(cm + " can not found resource id ");
            ...
            case PAYMENT_SALE_COMPLETED:
                ...
                if (!StringUtils.hasText(payAt)) throw new IllegalArgumentException(cm + " can not found resource payAt.");
